    ...
    <div class="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6 no-print">
            <form method="GET" action="{{ route('pdg.receptions') }}" id="filterForm">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-calendar-alt mr-1 text-amber-600"></i>
                        ...
                        <select name="producteur_id"
                            ...
                            @foreach($producteurs as $producteur)
                                <option value="{{ $producteur->id }}"
                                    {{ ($currentFilters['producteur_id'] ?? '') == $producteur->id ? 'selected' : '' }}>
                                    {{ $producteur->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="verrou"
                                class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg focus:border-red-500 focus:ring-2 focus:ring-red-200 transition-all"
                                onchange="document.getElementById('filterForm').submit()">
                            <option value="">{{ $isFrench ? 'Tous les statuts' : 'All statuses' }}</option>
                            <option value="0" {{ ($currentFilters['verrou'] ?? '') === '0' ? 'selected' : '' }}>
                                {{ $isFrench ? 'Non verrouillé' : 'Unlocked' }}
                            </option>
                            <option value="1" {{ ($currentFilters['verrou'] ?? '') === '1' ? 'selected' : '' }}>
                ...
                <div class="flex flex-wrap gap-3">
                    <button type="submit"
                            class="px-6 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-all shadow-md">
                        <i class="fas fa-filter mr-2"></i>{{ $isFrench ? 'Filtrer' : 'Filter' }}
                    </button>
                    <a href="{{ route('pdg.receptions.reset') }}"
                       class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-all shadow-md">
                        <i class="fas fa-redo mr-2"></i>{{ $isFrench ? 'Réinitialiser' : 'Reset' }}
                    </a>
                    <button type="button" onclick="exportToCSV()"
                            class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all shadow-md">
                        <i class="fas fa-file-csv mr-2"></i>{{ $isFrench ? 'Exporter CSV' : 'Export CSV' }}
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="table-container overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="receptionsTable">
                    <thead class="bg-gradient-to-r from-blue-600 to-blue-700 sticky-header">
                        <tr>
                        ...
                        <tr class="hover:bg-blue-50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col">
                                    ...
                                    @if($reception->est_fusion ?? false)
                                        <span class="text-xs text-indigo-500 font-semibold mt-1">
                                            <i class="fas fa-layer-group mr-1"></i>
                                            {{ count($reception->ids_fusionnes) }} {{ $isFrench ? 'entrées' : 'entries' }}
                                        </span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center justify-center px-3 py-1 rounded-full text-sm font-bold bg-blue-100 text-blue-800">
                                    {{ $reception->quantite_fusionnee ?? $reception->quantite }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="h-8 w-8 bg-gray-200 rounded-full flex items-center justify-center flex-shrink-0">
                                        <i class="fas fa-user text-gray-600 text-xs"></i>
                                    </div>
                                    <span class="ml-3 text-sm text-gray-900">{{ $reception->pointeur->name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($reception->vendeurAssigne)
                                    <div class="flex items-center">
                                        <div class="h-8 w-8 bg-green-200 rounded-full flex items-center justify-center flex-shrink-0">
                                            <i class="fas fa-user-tie text-green-600 text-xs"></i>
                                        </div>
                                        <span class="ml-3 text-sm text-gray-900">{{ $reception->vendeurAssigne->name }}</span>
                                    </div>
                                @else
                                    <div class="flex items-center">
                                        <div class="h-8 w-8 bg-amber-100 rounded-full flex items-center justify-center flex-shrink-0">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center no-print">
                                @if(!$reception->verrou)
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('pdg.receptions.edit', $reception->id) }}?ids_fusionnes={{ implode(',', $reception->ids_fusionnes ?? [$reception->id]) }}"
                                           class="inline-flex items-center px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-all shadow-sm">
                                            <i class="fas fa-edit mr-1"></i>
                                            {{ $isFrench ? 'Modifier' : 'Edit' }}
                                        </a>
                                        <button onclick="confirmDeleteReception({{ $reception->id }}, '{{ implode(',', $reception->ids_fusionnes ?? [$reception->id]) }}')"
                                                class="inline-flex items-center px-3 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-lg transition-all shadow-sm">
                                            <i class="fas fa-trash mr-1"></i>
                                            {{ $isFrench ? 'Supprimer' : 'Delete' }}
                                        </button>
                                    </div>
                                @else
                                    <span class="inline-flex items-center px-3 py-2 bg-red-50 text-red-700 text-xs font-medium rounded-lg border border-red-200">
        ...
        <form id="deleteReceptionForm" method="POST" style="display:none;">
            @csrf
            @method('DELETE')
            <input type="hidden" name="ids_fusionnes" id="deleteIdsFusionnes">
        </form>
    </div>
</div>
...
function setDateFilter(type) {
    ...
    if (type === 'today') {
        ...
        dateDebutInput.value = s;
        dateFinInput.value   = s;
    } else if (type === 'week') {
        const day  = today.getDay();
        const mon  = new Date(today); mon.setDate(today.getDate() - (day === 0 ? 6 : day - 1));
        const sun  = new Date(mon);   sun.setDate(mon.getDate() + 6);
        dateDebutInput.value = mon.toISOString().split('T')[0];
        dateFinInput.value   = sun.toISOString().split('T')[0];
    }
    document.getElementById('filterForm').submit();
}

function confirmDeleteReception(id, idsFusionnes) {
    const isFusion = idsFusionnes.split(',').length > 1;
    const message  = isFusion
        ? '{{ $isFrench ? "Ce groupe contient plusieurs entrées. Toutes seront supprimées !" : "This group contains multiple entries. All will be deleted!" }}'
        : '{{ $isFrench ? "Cette réception sera supprimée définitivement." : "This reception will be permanently deleted." }}';
    ...
    if (typeof Swal !== 'undefined') {
        Swal.fire({
            ...
            if (result.isConfirmed) submitDeleteReception(id, idsFusionnes);
        });
    } else {
        if (confirm(message)) submitDeleteReception(id, idsFusionnes);
    }
}

function submitDeleteReception(id, idsFusionnes) {
    const form = document.getElementById('deleteReceptionForm');
    form.action = `/pdg/receptions/${id}`;
    document.getElementById('deleteIdsFusionnes').value = idsFusionnes;
    form.submit();
}
...
function exportToCSV() {
    ...
    const headers = [
        '{{ $isFrench ? "Date" : "Date" }}','{{ $isFrench ? "Heure" : "Time" }}',
        '{{ $isFrench ? "Produit" : "Product" }}','{{ $isFrench ? "Prix" : "Price" }}',
        '{{ $isFrench ? "Quantité" : "Quantity" }}','{{ $isFrench ? "Pointeur" : "Pointer" }}',
        '{{ $isFrench ? "Vendeur" : "Seller" }}'
    ];
    ...
    rows.forEach(row => {
        ...
        const row_data = [
            c[0].querySelector('.font-medium')?.textContent.trim() ?? '',
            c[0].querySelector('.text-gray-500')?.textContent.trim() ?? '',
            c[1].querySelector('.font-semibold')?.textContent.trim() ?? '',
            c[1].querySelector('.text-gray-500')?.textContent.trim().replace(' FCFA','').replace('·','').trim() ?? '',
            c[2].querySelector('.font-bold')?.textContent.trim() ?? '',
            c[3].textContent.trim(),
            c[4].textContent.trim(),
        ];
        csv += row_data.map(v => '"' + v.replace(/"/g,'""') + '"').join(';') + '\n';
    });
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
    const a    = document.createElement('a');
    a.href     = URL.createObjectURL(blob);
    a.download = 'receptions_' + new Date().toISOString().split('T')[0] + '.csv';
    a.click();
}
</script>
@endsection